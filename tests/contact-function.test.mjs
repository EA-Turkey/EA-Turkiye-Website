...
test("sends notification through Resend to the fixed recipient and records sent status", async () => {
  ...
  await withMockedFetch(
    ...
    async (fetchCalls) => {
      ...
      assert.match(body.text, /^EA Turkiye website contact form/m);
      assert.match(body.text, /^SUMMARY$/m);
      assert.match(body.text, /^Name: Ada Lovelace$/m);
      assert.match(body.text, /^Email: ada@example.com$/m);
      assert.match(body.text, /^DETAILS$/m);
      assert.match(body.text, /^Organization: Analytical Engines$/m);
      assert.match(body.text, /^MESSAGE$/m);
      assert.match(body.text, /Submission ID: 1/);
      assert.match(body.text, /Source: https:\/\/example.com\/en\/contact\//);
