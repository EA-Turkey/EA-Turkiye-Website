---
translationKey: get-involved
title: Get involved
description: Ways to join the Effective Altruism Turkey community — programs, events, and career coaching.
url: /en/get-involved/
hero_eyebrow: Get involved
hero_title: Join the community
hero_lead: Whether you're new to effective altruism or experienced, there are many ways to learn, contribute, and meet people who share your values.
action_cards_text: Start with the path that fits you best.
action_cards:
  - title: Introduction Program
    badge: Online
    text: A free online program where you discuss the core ideas of effective altruism in small, facilitated groups.
    cta_label: Application form →
    cta_url: "https://www.effectivealtruism.org/courses/introductory-program"
  - title: AI Safety Program
    badge: Online
    text: Interested in the safe development of AI? Register your interest in our AI safety program.
    cta_label: Interest form →
    cta_url: "https://docs.google.com/forms/d/e/1FAIpQLScW-UALwTNUDAOI7xPDui7iSZ-OTIZhMzL1NMbyHDfN8Emi3Q/viewform?usp=header"
  - title: Career Coaching
    badge: One-on-one
    text: Get individual coaching to find high-impact career paths aligned with your skills, interests, and values.
    cta_label: Application form →
    cta_url: "https://docs.google.com/forms/d/e/1FAIpQLSdIXyOBIyhYBSCWcGA4XARX1FqGUXUQoBB-prHcV-bQvYnrbQ/viewform?usp=header"
  - title: Events
    badge: In person and online
    text: From reading groups to summits — get to know the community by joining an event that matches your interests.
    cta_label: View events →
    cta_url: "/en/events/"
form:
  heading: Write to us
  lead: Not sure where to start? Introduce yourself briefly and we'll figure it out together.
  interests:
    - General question
    - Joining the community
    - Events or programs
    - Collaboration
    - Volunteering
contact_people_text: You can always reach out to us with questions.
contact_ids:
  - devrim
  - firat
faq_text: ""
faq:
  - question: Does participation cost anything?
    answer: No. Our programs and events are free.
  - question: Who can join?
    answer: Students, researchers, professionals — anyone who wants to do good more effectively is welcome; there are no prerequisites.
  - question: What language are the programs in?
    answer: Most of our events are held in Turkish; some programs and resources may be in English. Each program's language is stated on its application page.
  - question: How much time does it take?
    answer: Programs typically involve a few hours of reading and discussion per week; you can join events as often as you like.
---
